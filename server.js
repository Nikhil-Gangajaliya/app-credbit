...
app.post('/api/change-credentials', (req, res) => {
  ...
  if (!user || !bcrypt.compareSync(oldPassword, user.password_hash)) {
    return res.status(401).json({ error: 'Invalid old credentials' });
  }
...
app.post('/api/entry', (req, res) => {
  ...
  const trx = db.transaction(() => {
    ...
    db.prepare(`
      ...
      VALUES (?,?,?,?,?,?,?)
    `).run(date, party.id, party.name, purpose || '', parseFloat(debit || 0), parseFloat(credit || 0), reference || '');
